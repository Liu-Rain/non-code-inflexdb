...
def resultquery(request):
    ...
    data = f'''from(bucket: "{bucket}")
        |> range(start: -48h)
        |> filter(fn: (r) => r._measurement == "{meansurement}")
    ...
    return JsonResponse({"messages": f'{response.text}', "code": f'{response.status_code}', "cookies" : f'{cookies}', "bucket": bucket, "meansurement":meansurement, "field":field ,"data": rows}, status = 200)
